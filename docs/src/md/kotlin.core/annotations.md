...
> Important: an annotation type cannot reference itself, either directly or indirectly, unless every such reference cycle includes an array type.
> For example, if annotation type `A` references annotation type `B` which references `A`, it is prohibited and reported as a compile-time error.
> If `B` references an array of `A` instead, the cycle is allowed.
