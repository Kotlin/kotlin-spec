...
> Important: an annotation type cannot reference itself, either directly or indirectly, unless every such reference cycle includes an array-typed or `vararg` parameter.
> For example, if annotation type `A` has a parameter of annotation type `B`, which has a parameter of type `A`, it is prohibited and reported as a compile-time error.
> If the parameter of `B` has type `Array<A>` or is declared as `vararg` of type `A` instead, the cycle is allowed.
...
- Any parameter of this function of a [function type][Function types] is treated as *inlined* parameter unless it has one of two special modifiers: `crossinline` or `noinline`.
  If a particular argument corresponding to an inline parameter is a [function literal][Function literals], this function literal is considered *inlined* and, in particular, affects the way [return expressions], [continue expressions] and [break expressions] are handled in its body.
