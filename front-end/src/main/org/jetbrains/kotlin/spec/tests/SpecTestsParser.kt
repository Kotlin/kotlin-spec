...
object SpecTestsParser {
    ...
    private val directiveRegex = SINGLE_LINE_COMMENT_REGEX.format("[\\w\\s]+TESTCASE NUMBER:") +
    ...
    fun parseSpecTest(response: Map<String, Any>, testTypeInfo: TestTypeInfo): Map<String, Any?> {
        val data = response[testTypeInfo.content].toString()
                .replace(diagnosticTagRegexp, "")
                .replace(implementationTestInfoPattern, "")
                .replace(topLevelDirectivePattern, "")
        return when (testTypeInfo) {
            TestTypeInfo.DIAG -> getInfoForImplementationTestDiag(data)
            TestTypeInfo.BOX -> getInfoForImplementationTestBox(data)
        }
    }

    private fun getInfoForImplementationTestDiag(data: String): Map<String, Any?> {
        ...
        while (testCaseMatches != null) {
            ...
            val code = (testCaseMatches.groups[2] ?: testCaseMatches.groups[5])?.value?.replace(diagnosticTagRegexp, "")
                    ?: continue
            startPosition += testCaseMatches.range.last - (testCaseMatches.groups[3]
                    ?: testCaseMatches.groups[6])?.value?.length!!
    ...
    private fun getInfoForImplementationTestBox(data: String): Map<String, Any?> {
        val testInfoElements =
                parseTestInfoElements(testInfoPattern.find(data)?.groups?.get(3)?.value
                        ?: throw Exception("Unable to parse test"))
